void desenhaQuiz(PGraphics g) {
  ...
  // Nível Médio
  if (nivelQuiz == 1) {
    if (perguntaAtual == 0) g.text("Qual planeta leva mais tempo\npara orbitar o Sol?", width/2, height*0.26);
    if (perguntaAtual == 1) g.text("Qual e o planeta\nmais quente do Sistema Solar?", width/2, height*0.26);
    if (perguntaAtual == 2) g.text("Qual planeta possui a maior\nquantidade de luas conhecidas?", width/2, height*0.26);
    if (perguntaAtual == 3) g.text("Qual e a maior\nlua de Jupiter?", width/2, height*0.26);
    if (perguntaAtual == 4) g.text("Qual e a principal\ncomposicao do Sol?", width/2, height*0.26);
  }

  // Nível Difícil
  if (nivelQuiz == 2) {
    if (perguntaAtual == 0) g.text("A lua Europa tem oceano sob\nsua superficie. A qual planeta ela pertence?", width/2, height*0.26);
    if (perguntaAtual == 1) g.text("Qual objeto foi considerado\no nono planeta ate 2006?", width/2, height*0.26);
    if (perguntaAtual == 2) g.text("Qual planeta possui o maior\ncampo magnetico do Sistema Solar?", width/2, height*0.26);
    if (perguntaAtual == 3) g.text("Qual planeta gira deitado,\ncom o eixo paralelo a sua orbita?", width/2, height*0.26);
    if (perguntaAtual == 4) g.text("Qual planeta tem um dia\nmais longo que seu proprio ano?", width/2, height*0.26);
  }
...
// Nível Fácil
  ...
  // Nível Médio
  if (nivelQuiz == 1) {
    if (perguntaAtual == 0) desenhaOpcoes(g, "Jupiter", "Marte", "Saturno", "Netuno", 3);
    if (perguntaAtual == 1) desenhaOpcoes(g, "Mercurio", "Marte", "Venus", "Terra", 2);
    if (perguntaAtual == 2) desenhaOpcoes(g, "Jupiter", "Urano", "Netuno", "Saturno", 3);
    if (perguntaAtual == 3) desenhaOpcoes(g, "Callisto", "Europa", "Ganimedes", "Io", 2);
    if (perguntaAtual == 4) desenhaOpcoes(g, "Oxigenio", "Hidrogenio", "Helio", "Nitrogenio", 1);
  }

  // Nível Difícil
  if (nivelQuiz == 2) {
    if (perguntaAtual == 0) desenhaOpcoes(g, "Saturno", "Jupiter", "Urano", "Netuno", 1);
    if (perguntaAtual == 1) desenhaOpcoes(g, "Eris", "Ceres", "Plutao", "Makemake", 2);
    if (perguntaAtual == 2) desenhaOpcoes(g, "Saturno", "Terra", "Netuno", "Jupiter", 3);
    if (perguntaAtual == 3) desenhaOpcoes(g, "Saturno", "Netuno", "Urano", "Marte", 2);
    if (perguntaAtual == 4) desenhaOpcoes(g, "Mercurio", "Venus", "Marte", "Jupiter", 1);
  }
}
